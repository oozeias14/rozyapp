...
async function run() {
  ...
  allMsgsList.forEach((m) => {
    const jids = [m.key?.remoteJid, m.key?.remoteJidAlt, m.key?.participant, m.key?.participantAlt, m.remoteJid, m.remoteJidAlt].filter(Boolean);
    jids.forEach((j) => {
      if (j.includes('@g.us') || j.includes('@broadcast')) return;
      let clean = extractCleanPhone(j);
      if (lidToPhone.has(clean)) clean = lidToPhone.get(clean);
  ...
  console.log('Total matched signatures:', matchedSigs.size);

  const testContacts = [
    { name: 'Kauan', phone: '61992419984' },
    { name: 'Kamilla', phone: '61994409686' },
    { name: 'Rozy Costa', phone: '61992623060' }
  ];

  testContacts.forEach(tc => {
    const sigs = getPhoneSignatures(tc.phone);
    const is2Checks = sigs.some(s => matchedSigs.has(s));
    console.log(`Contato: ${tc.name} (${tc.phone}) -> ${is2Checks ? '✅ 2 TRAÇOS (SALVO)' : '❌ 1 TRAÇO (PENDENTE)'}`);
  });
}
