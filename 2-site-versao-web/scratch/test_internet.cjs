...
(async () => {
  ...
  console.log("Matching Message IDs for 'Internet':", Array.from(matchingMessageIds));
  ...
  statusRecords.forEach(sr => {
    ...
    if (matchesKey || matchesMsg) {
      console.log(`Matched Status Record: keyId=${sr.keyId}, participant=${sr.participant}, remoteJid=${sr.remoteJid}, status=${sr.status}`);
      [sr.participant, sr.remoteJid, sr.fromMeJid, sr.participantAlt, sr.remoteJidAlt].forEach(j => {
        ...
        if (lidToPhone.has(clean)) {
          console.log(`Resolved LID ${clean} to Phone ${lidToPhone.get(clean)}`);
          clean = lidToPhone.get(clean);
        }
  ...
  const rozySigs = getPhoneSignatures("61992623060");

  console.log("Kauan (6192419984) in matchedSigs:", kauanSigs.some(s => matchedSigs.has(s)));
  console.log("Kamilla (6194409686) in matchedSigs:", kamillaSigs.some(s => matchedSigs.has(s)));
  console.log("Rozy (61992623060) in matchedSigs:", rozySigs.some(s => matchedSigs.has(s)));
})();
