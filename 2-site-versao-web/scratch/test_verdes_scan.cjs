...
async function testScan(phrase) {
  console.log(`\n==============================================`);
  console.log(`🔍 RUNNING SCAN FOR PHRASE: "${phrase}"`);
  console.log(`==============================================`);
  ...
  console.log(`Matching message IDs for "${phrase}":`, Array.from(matchingMessageIds));
  ...
  statusRecords.forEach((sr) => {
    ...
    if (matchesKey || matchesMsg) {
      console.log(`Status record matched:`, sr);
      const st = (sr.status || '').toUpperCase();
      if (st === 'DELIVERY_ACK' || st === 'READ' || st === 'PLAYED' || st === 'SERVER_ACK' || !st) {
