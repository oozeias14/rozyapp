...
function doesMatchPhrase(text, targetPhrase) {
  if (!targetPhrase) return true; // If no phrase specified, match all
...
(async () => {
  console.log("=== Testing Auto Broadcast Tracking (with and without phrase) ===");
  ...
  // Test with phrase "Internet" and without phrase (automatic)
  const testCases = [
    { name: "With Phrase 'Internet'", phrase: "Internet" },
    { name: "Automatic All Broadcasts (No Phrase)", phrase: "" }
  ];
  ...
  for (const tc of testCases) {
    ...
    allMsgs.forEach(m => {
      ...
      const matches = tc.phrase ? doesMatchPhrase(text, tc.phrase) : isBroadcast;
      if (matches && isRecent(m, 2)) {
    ...
    allMsgs.forEach(m => {
      const text = extractActualText(m);
      const matches = tc.phrase ? doesMatchPhrase(text, tc.phrase) : false;
      if (matches && isRecent(m, 2)) {
    ...
    // Check Kauan with his Supabase whatsapp: 61992419984 / 6192419984
    const kauanSigs1 = getPhoneSignatures("61992419984");
    const kauanSigs2 = getPhoneSignatures("6192419984");
    console.log(`Kauan (61992419984 from DB) matched:`, kauanSigs1.some(s => matchedSigs.has(s)));
    console.log(`Kauan (6192419984) matched:`, kauanSigs2.some(s => matchedSigs.has(s)));
  }
})();
