...
function isWithinMinutes(msg, maxMinutes = 30) {
  if (!msg) return false;
  ...
  const diffMinutes = (nowSec - ts) / 60;
  return diffMinutes >= -1.0 && diffMinutes <= maxMinutes;
}

(async () => {
  console.log("=== Testing 30-Minute Universal Window ===");
  const [contactsRes, chatsRes] = await Promise.all([
    post("/chat/findContacts/dr_candido", {}),
    post("/chat/findChats/dr_candido", {})
  ]);
  ...
  allMsgs.forEach(m => {
    if (isWithinMinutes(m, 120)) { // testing last 120m for current mock time
      if (m.key?.id) recentMsgIds.add(m.key.id);
      if (m.id) recentMsgIds.add(m.id);
      ...
      [m.key?.remoteJid, m.remoteJid, m.key?.remoteJidAlt, m.remoteJidAlt, m.key?.participant, m.participant].forEach(j => {
        ...
        if (clean && clean.length >= 8) getPhoneSignatures(clean).forEach(sig => matchedSigs.add(sig));
      });
    }
  });

  // Extract from status receipts
  statusRecords.forEach(sr => {
    if (sr.keyId && recentMsgIds.has(sr.keyId)) {
  ...
  // Extract from active chats
  chats.forEach(c => {
    if (isWithinMinutes(c, 120)) {
  ...
  console.log(`Matched ${matchedSigs.size} signatures from recent messages/receipts!`);
  const kauanSigs = getPhoneSignatures("61992419984");
  console.log("Kauan (61992419984) detected:", kauanSigs.some(s => matchedSigs.has(s)));
})();
