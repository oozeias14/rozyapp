...
(async () => {
  const users = await getSupabase("/rest/v1/users?select=id,name,phone,whatsapp&limit=100");
  console.log(`Fetched ${Array.isArray(users) ? users.length : 0} users from Supabase.`);
