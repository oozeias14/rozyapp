...
export default function FirstAccessModal({ profile, onClose }) {
  ...
  async function handleSaveCandidateContact() {
    let tel = (adminPhone || '').replace(/\D/g, '');
    let intlTel = tel;
    ...
    const fileName = 'contato_dr_candido.vcf';
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Dr. Cândido',
          text: 'Salvar contato oficial do Dr. Cândido'
        });
      } else {
      ...
      console.log('Share/download error:', e);
    }

    // Registra no banco que o usuário baixou o contato do Dr. Cândido
    try {
      await supabase.from('profiles').update({ contact_saved: true }).eq('id', profile.id);
    } catch (err) {
      console.error('Error updating contact_saved:', err);
    }
  }

  async function handleSendWhatsapp() {
    // Registra no banco que o usuário enviou a mensagem de primeiro acesso
    try {
      await supabase.from('profiles').update({ first_access_sent: true }).eq('id', profile.id);
    } catch (err) {
      console.error('Error updating first_access_sent:', err);
    }
    ...
    <div className="modal-bg" style={{ zIndex: 11000 }}>
      <div className="modal" style={{ maxWidth: 400, padding: 24, textAlign: 'center', backgroundColor: '#090d16', borderColor: 'var(--line)' }}>
        ...
        <p style={{ color: 'var(--ink2)', fontSize: 13, marginBottom: 18, lineHeight: 1.5 }}>
          Olá, <strong style={{ color: 'var(--ink1)' }}>{profile.name}</strong>! Salve o contato do Dr. Cândido e nos envie uma mensagem no WhatsApp para ativar seu acesso completo:
        </p>

        <div style={{ background: 'var(--panel2)', borderRadius: 12, padding: 14, marginBottom: 18, textAlign: 'left', border: '1px solid var(--line)' }}>
        ...
        {timeLeft > 0 ? (
          <div style={{ fontSize: 12, color: 'var(--warn)', marginBottom: 14, fontWeight: '700' }}>
            ⏱️ Aguarde {formatSeconds(timeLeft)} para fechar...
          </div>
        ) : (
          <div style={{ fontSize: 12, color: 'var(--teal)', marginBottom: 14, fontWeight: '700' }}>
            ✅ Tempo finalizado. Você já pode fechar.
          </div>
        )}

        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12 }}>
          {/* Passo 1: Salvar Contato */}
          <button 
            type="button"
          ...
          >
            <span>📥</span>
            <span>1. Salvar Dr. Cândido na Agenda</span>
          </button>
          ...
          >
            ...
            <span>2. Enviar Mensagem no WhatsApp</span>
          </button>
        </div>
        ...
        <button 
          ...
          style={{ width: '100%', margin: 0, opacity: timeLeft > 0 ? 0.4 : 1, cursor: timeLeft > 0 ? 'not-allowed' : 'pointer', fontSize: 12.5 }}
        >
          Fechar
        </button>
      </div>
