  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (loginEmail && !loginEmail.includes('@')) {
        ...
        // 1. Tenta buscar por WhatsApp (aceita 061..., 61..., 993106490, 992623060, formatado, etc.)
        if (digitsOnly.length >= 8) {
          ...
          let full11 = clean;
          if (clean.length === 8) full11 = '619' + clean;
          else if (clean.length === 9) full11 = '61' + clean;
          else if (clean.length === 10 && clean.startsWith('61')) full11 = '619' + clean.slice(2);

          const nineDigits = full11.slice(-9);
          const part1 = nineDigits.slice(1, 5);
          const part2 = nineDigits.slice(5);
          ...
          const orClauses = [
            'whatsapp.in.(' + variants.join(',') + ')',
            'phone.in.(' + variants.join(',') + ')',
            'whatsapp.ilike.%' + part1 + '%' + part2 + '%',
            'phone.ilike.%' + part1 + '%' + part2 + '%'
          ];

          const { data: phoneMatches, error: phoneErr } = await supabase
            .from('profiles')
            .select('id, email, username, whatsapp, phone')
            .or(orClauses.join(','))
            .limit(1);

          if (!phoneErr && phoneMatches && phoneMatches.length > 0) {
            foundProfile = phoneMatches[0];
          }
        }
